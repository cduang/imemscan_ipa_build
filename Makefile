...
build:
	...
	# 创建 TIPA 文件 (TrollStore 安装包)
	@echo "Creating TIPA file for TrollStore..."
	@cp build/iMemScan.ipa build/iMemScanTS.tipa
	...
	@echo "TIPA file: build/iMemScanTS.tipa"
...
# TrollStore 目标 - 只构建 TIPA 文件
trollstore:
	@echo "Building iMemScanTS for TrollStore..."
	@mkdir -p $(APP_TMP)
	...
	@cd build && zip -r9 iMemScanTS.tipa Payload
